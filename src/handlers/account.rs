...
use crate::{
    ...
    models::{AppointmentOverviewRow, CustomerSummaryRow, Payment},
    state::AppState,
    utils::{
        appointment_status_label, format_cents, format_datetime, now_utc, payment_status_label,
    },
    views::{AppointmentView, CustomerProfileView, PageShell, PaymentView},
};

use super::{build_shell, render};

pub async fn dashboard(State(state): State<AppState>, jar: CookieJar) -> AppResult<Response> {
    let current_user = auth_helpers::require_customer(auth_helpers::require_login(
